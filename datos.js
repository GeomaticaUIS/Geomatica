...
const objetoJSONGuardado = localStorage.getItem(dato);
 ...
 const servidor =objetoRecuperado.RUTA_ORTOMOSAICO; //"\\\\servidor\\carpetacompartida";
 const nombreArchivo = objetoRecuperado.NOMBRE_ORTOMOSAICO//"documento.pdf"; 
 const LINK= `${servidor}\\${nombreArchivo}`;
 const enlaceDescargar = document.createElement('a');
 enlaceDescargar.textContent = 'Descarga el LAZ o ORTOMOSAICO';
 enlaceDescargar.href = LINK;
 enlaceDescargar.download = nombreArchivo;
 const spans =document.getElementById("archivo");
